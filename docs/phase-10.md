...
- `listings.unused_quantity` / `listings.used_quantity`: Queensland-only split of `quantity` for FDU1465 unused/used columns (FDU1469 fills unused). They must add up to `quantity`. Create listing and create auction collect unused and used; the split is not shown elsewhere yet. Queensland seed listings and status fixtures store unused = quantity and used = 0. Orders snapshot the same split.
