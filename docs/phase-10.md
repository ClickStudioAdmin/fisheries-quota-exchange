...
| `/orders/[id]` | During `AWAITING_COMPLIANCE`: payment-received notice. Status bar is quota, payment, compliance, transfer, then settlement. Request-update notes open from View Message. During `AWAITING_TRANSFER` on a QLD order: status, missing fields, prepare/download unsigned PDF |
