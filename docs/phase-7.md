...
5. Admin simulates settlement (`COMPLETED`): seller ledger `SALE` or `LEASE_OUT`, buyer ledger `PURCHASE` or `LEASE_IN`, reservation `CONSUMED`, listing `SOLD`, transaction `COMPLETED`. The app then emails dummy tax invoice PDFs to the buyer (`created_by_email`): quota (seller to buyer) and platform fee (FQX to seller). Buyer and seller can download both from `/orders/[id]` after settlement. Settlement still completes if mail is skipped or fails.
