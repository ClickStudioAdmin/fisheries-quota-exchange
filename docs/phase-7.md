...
5. Admin simulates settlement (`COMPLETED`): seller ledger `SALE` or `LEASE_OUT`, buyer ledger `PURCHASE` or `LEASE_IN`, reservation `CONSUMED`, listing `SOLD`, transaction `COMPLETED`.
